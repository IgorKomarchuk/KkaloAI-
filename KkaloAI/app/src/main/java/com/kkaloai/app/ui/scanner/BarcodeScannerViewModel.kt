...
class BarcodeScannerViewModel @Inject constructor(
    ...
    private val foodCache: com.kkaloai.app.data.remote.FoodCacheRepository
) : ViewModel() {
    ...
    fun onBarcodeScanned(barcode: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                foodCache.lookupBarcode(barcode)?.let { cached ->
                    achievementEngine.onBarcodeScan()
                    _scanState.value = ScannerUiState.Success(cached)
                    return@launch
                }
                ...
                if (response.status == 1 && response.product != null) {
                    ...
                    achievementEngine.onBarcodeScan()
                    val payload = GeminiFoodResponse(
                        items = listOf(result),
                        totalCalories = result.calories,
                        summary = "Barcode scan result for $barcode"
                    )
                    foodCache.saveBarcode(barcode, payload)
                    _scanState.value = ScannerUiState.Success(payload)
                } else {
